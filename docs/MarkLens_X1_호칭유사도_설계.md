...
| 분리관찰 | 제거 후 각 토큰의 2음절 이상 읽기를 독립 후보로. 알파벳·숫자 1글자 토큰 읽기(K→케이, 7→세븐)는 제외. 제거 후 4토큰 이상 슬로건형이면 영문·숫자 유래 읽기는 앞 `SLOGAN_LEAD_TOKENS`(2)개 토큰만 남기고, **한글 토큰은 항상 유지**(⑤ 병기 표장은 한글로 호칭). 띄어쓰기 없이 붙은 결합어는 후보 쌍의 접두·접미 포함 검사(`CONTAIN_*`)로 보완 | `_candidates_cached` (c), `_contain_sim` |
| 불가분 결합 예외 | 제거 전 토큰 전체 결합음을 항상 후보에 유지(1글자 토큰 읽기도 여기에는 포함) | `_candidates_cached` (a) |
| 요부관찰 | 회사 형태·부가어·기능어 제거(`UNIVERSAL_GENERIC`) + 호출자 전달 `extra_generic`. `extra_generic` 은 원 토큰뿐 아니라 토큰의 읽기(COFFEE→커피)와도 대조 | `_normalize_tokens`, `_reads_as_generic` |
...
| `KOREAN_BRAND_ROMANIZATION` | 국내 브랜드 로마자 **101개** (`korean_brands.py`) | 룰로 읽을 수 없는 한국어 로마자(samsung, hyundai …). 씨앗 110개 중 룰 결과와 같은 33개(kakao, naver …)는 중복이라 제외한 77개 + 로마자 캐기 승인분 10개(gudaero, chengdamsu, yumbbokki, nakwon, luciel, netmarble, petitzel, heyalfun, honsul, sancheon) + 유명 브랜드 목록 교차 확인 4개(sangmidang, eland, woowa, baedal) + v1.4.1 로마자 캐기 재분류 승인 10개(barun, youngmi, maemaeland, makerssaem, rairice, pilalier, median, defat, husic, regen). 룰보다 먼저 적용 |
...
| `G2P_EXCEPTIONS` | 기능어·수사·약어 24개 + 외래어 관용 표기 39개 = 63개 | 룰로 맞추기 어려운 불규칙. 브랜드(hyundai/lotte/kia)는 브랜드 표로 이동. 관용 표기(pilates, ballet, etoile, curry, atelier, gourmet, poke, musical, salon, baseball, together)는 로마자 캐기에서 확인해 승인, coffee→커피 는 룰의 o→ㅗ 기본값이 맞지 않는 불규칙, paris·baguette·republica 는 유명 브랜드 목록 교차 확인에서 승인. v1.4: 벤치마크 60단어 검수에서 룰로 일반화할 수 없는 hot·good·big·love·living·express·bio 와 leisure·electro·saturday, 모음 사이 s 유성음 규칙의 예외 basic·asia·asian·genesis·research·crisis·evisu. v1.4.1: 캐기 재분류 승인 santa·soccer·academy·guardians·eat·chinese·business |
...
| `CONTAIN_MIN_SYLLABLES` / `CONTAIN_MIN_COVERAGE` / `CONTAIN_BASE_SCORE` | 3 / 0.5 / 0.9 | 붙여쓴 결합어 포함 검사: 후보 쌍에서 짧은 쪽이 긴 쪽의 접두·접미이고 3음절 이상, 길이 비율 0.5 이상이면 포함 점수 0.9 + 0.1×비율. 2음절 접두(스타/스타벅스)는 불가분 결합으로 보아 제외 |
...
| `잇버거 EAT PREMIUM BURGER` | 잇버거이트프레미엄버거, 잇버거이에이티프레미엄버거, 잇버거, 이트, 이에이티 | 4토큰 슬로건형(v1.3.1): 한글 잇버거는 유지, 영문은 앞 2개 토큰(eat)까지만 단독 후보. `잇버거` 대비 1.000 |
...
5. **붙여쓴 한글 토큰 안의 보통명칭 접미**(바른치과의 치과)는 X1 단독으로 분리 불가("바른치과 / BARUN DENTAL" 0.636) — 식별력 필터 통합 시 접미 분리 규칙으로 해결 예정.
6. **불가분 결합은 근사**: 결합 여부를 판단하지 않고 "전체 결합음을 항상 후보에 유지"로 대신한다.
7. **낱자 읽기 상한 3자**: 모음이 있는 4자 이상 약어(NASA, ASUS 등)는 룰 읽기만 붙는다(나사, 아수스). 모음 없는 약어(KBS, HSBC)는 길이와 무관하게 낱자 읽기가 붙는다.
...
### 5-4. `extra_generic` 매칭 범위 — 결정 완료 (2026-09-18, v1.3.1)
